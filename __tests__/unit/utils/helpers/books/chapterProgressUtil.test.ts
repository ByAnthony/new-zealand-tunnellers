...
describe("chapterProgressUtil", () => {
  ...
  test("saves and gets chapter progress using normalized pathname", () => {
    saveChapterProgress("/kiwis-dig-tunnels-too/prologue/", 15.2);

    expect(getChapterProgress("kiwis-dig-tunnels-too/prologue")).toBe(15);
    expect(getChapterProgress("/kiwis-dig-tunnels-too/prologue/")).toBe(15);
  });

  test("does overwrite existing progress when new progress is 0", () => {
    saveChapterProgress("/kiwis-dig-tunnels-too/prologue/", 15.2);
    saveChapterProgress("/kiwis-dig-tunnels-too/prologue/", 0);

    expect(getChapterProgress("/kiwis-dig-tunnels-too/prologue/")).toBe(0);
  });

  test("returns 0 when chapter is not found", () => {
    expect(getChapterProgress("/kiwis-dig-tunnels-too/chapter-1/")).toBe(0);
  });
  ...
  test("does not throw when localStorage.setItem throws in saveChapterProgress", () => {
    ...
    expect(() =>
      saveChapterProgress("/kiwis-dig-tunnels-too/prologue/", 50),
    ).not.toThrow();
  });
});
